...
describe("GET /items", async function () {
    test("Gets alist of items", async function () {
 ...
 desrcibe ("GET /items/:name", async function () {
    test("Get a single item", async function () {
        const response = await request(app).app(`/items/items/${item.name}`);
        expect(response.statusCode).toBe(200);
 ...
 describe("POST / items", async function () {
    test("Create a new item", async function () {
        ...
        expect(response.stautsCode).toBe(200);
        expect(response.body.item).toHaveProperty("name")
        expect(response.bodt.item).toHaveProperty("price")
